            ...
            try:
                ...
                while utilisation > self.quota:
                    ...
                    async with self._db.execute("SELECT id FROM events ORDER BY end ASC LIMIT 1") as event_cursor:
                        ...
                        if row is None:
                            logger.warning(
                                "Storage quota exceeded, but there are no events in the database - Do you have stray files?"
                            )
                            break
                        event_id = row[0]
                        ...
                        if (
                            ...
                            logger.info(
                                f"Storage quota {human_readable_size(utilisation)}/{human_readable_size(self.quota)} exceeded, purging oldest events"
                            )
                ...
                if deleted_a_file:
                    await tidy_empty_dirs(self.rclone_destination)
                    logger.info(
                        f"Storage utlisation back below quota limit: {human_readable_size(utilisation)}/{human_readable_size(self.quota)}"
                    )
